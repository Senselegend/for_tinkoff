...
void SetupOffsets()
{
	...
	offys.InvalidateBoneCache = Utilities::Memory::FindPatternAimbot("\x80\x3D\x00\x00\x00\x00\x00\x74\x16\xA1\x00\x00\x00\x00\x48\xC7\x81", "xx?????xxx????xxx", 0, "client.dll");
...
//	offys.m_hObserverTarget = g_pNetVars->GetOffset(XorStr("DT_BasePlayer"), XorStr("m_hObserverTarget"));
	...
	offys.getSequenceActivity = (DWORD)FindPatternIDA("client.dll", "55 8B EC 83 7D 08 FF 56 8B F1 74 3D");
	...
	offys.m_bClientSideAnimation = g_pNetVars->GetOffset("DT_CSPlayer", "m_bClientSideAnimation");
	offys.dwComputeHitboxSurroundingBox = FindPatternIDA("client.dll", "E9 ? ? ? ? 32 C0 5D");
