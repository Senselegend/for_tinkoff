...
bool CAutowall::HandleBulletPenetration(CSWeaponInfo* wpn_data, FireBulletData& data)
{
	...
	data.current_damage *= (float)pow(wpn_data->GetRangeModifier(), (data.trace_length * 0.002));
	...
	auto flTakenDamage = (data.current_damage * final_damage_modifier) + flModifier * 3.f * fmaxf(0.f, (3.f / wpn_data->GetPen()) * 1.25f);
...
bool CAutowall::SimulateFireBullet1(FireBulletData &data, CBaseEntity *player, int hitbox)
{
	...
	auto *weaponData = G::MainWeapon->GetCSWpnData();
	...
	while ((data.penetrate_count > 0) && (data.current_damage >= 1.0f))
	{
		...
		if (data.trace_length > 3000.f && weaponData->GetPen() > 0.f || enter_surf_penetration_mod < 0.1f)
			break;
		...
		if ((data.enter_trace.hitgroup <= 7) && (data.enter_trace.hitgroup > 0) && (data.enter_trace.m_pEnt->GetTeamNum() != G::LocalPlayer->GetTeamNum()) && pClass->m_ClassID != (int)CSGOClassID::CHostage) {
			data.trace_length += data.enter_trace.fraction * data.trace_length_remaining;
			data.current_damage *= (float)pow(weaponData->GetRangeModifier(), data.trace_length * 0.002);

			ScaleDamage(data.enter_trace.hitgroup, data.enter_trace.m_pEnt, weaponData->GetArmorRatio(), data.current_damage);
...
bool CAutowall::SimulateFireBullet(CBaseEntity* pBaseEntity, FireBulletData& data)
{
	...
	data.current_damage = (float)wpn_data->GetDamage();

	while ((data.penetrate_count > 0) && (data.current_damage >= 1.0f)) {
		data.trace_length_remaining = wpn_data->GetRange() - data.trace_length;
		...
		if ((data.enter_trace.hitgroup <= 7) && (data.enter_trace.hitgroup > 0) && (data.enter_trace.m_pEnt->GetTeamNum() != G::LocalPlayer->GetTeamNum()) && pClass->m_ClassID != (int)CSGOClassID::CHostage) {
			...
			ScaleDamage(data.enter_trace.hitgroup, data.enter_trace.m_pEnt, wpn_data->GetArmorRatio(), data.current_damage);
...
bool CAutowall::PenetrateWall(CBaseEntity* pBaseEntity, const Vector& vecPoint)
{
	...
	if (SimulateFireBullet(G::LocalPlayer, data)) {
		...
		float min_damage = Clientvariables->Ragebot.Mindamage;
...
bool CAutowall::CanHit(CBaseEntity* entity, const Vector &point, float *damage_given)
{
	...
	if (SimulateFireBullet(entity, data))
	{
...
bool CAutowall::CanWallBang(float& dmg)
{
	...
	if (m_local && m_local->isAlive()) {
		...
		if (m_weapon) {
			...
			if (weapon_data) {
				data.current_damage = weapon_data->GetDamage();
				data.trace_length_remaining = weapon_data->GetRange() - data.trace_length;
				Vector end = data.src + data.direction * data.trace_length_remaining;
