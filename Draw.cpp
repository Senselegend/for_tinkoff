...
void CDraw::Init()
{
	...
	g_pSurface->SetFontGlyphSet(font_espnum, XorStr("Smallest Pixel-7"), 10, 100, 0, 0, FONTFLAG_OUTLINE);

	font_espinfo = g_pSurface->SCreateFont();
	g_pSurface->SetFontGlyphSet(font_espinfo, XorStr("Smallest Pixel-7"), 10, 100, 0, 0, FONTFLAG_OUTLINE);

	font_build = g_pSurface->SCreateFont();
	g_pSurface->SetFontGlyphSet(font_build, XorStr("Verdana"), 28, 650, 0, 0, FONTFLAG_DROPSHADOW | FONTFLAG_ANTIALIAS);

	aa_font = g_pSurface->SCreateFont();
	g_pSurface->SetFontGlyphSet(aa_font, XorStr("Calibri"), 46, 900, 0, 0, NULL);

	font_esp2 = g_pSurface->SCreateFont();
	g_pSurface->SetFontGlyphSet(font_esp, XorStr("Smallest Pixel - 7"), 10, 100, 0, 0, FONTFLAG_OUTLINE);

	icon_esp = g_pSurface->SCreateFont();
	g_pSurface->SetFontGlyphSet(icon_esp, XorStr("AstriumWep"), 11, 400, 0, 0, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);

	event_font = g_pSurface->SCreateFont();
	g_pSurface->SetFontGlyphSet(event_font, XorStr("Smallest Pixel-7"), 11, 100, 0, 0, FONTFLAG_OUTLINE);
...
void CDraw::Text2(int x, int y, Color color, DWORD font, const wchar_t* text)
{
...
void CDraw::textold(int x, int y, const wchar_t* _Input, int font, Color color)
{
	g_pSurface->DrawSetTextFont(font);
	g_pSurface->DrawSetTextColor(color);
	g_pSurface->DrawSetTextPos(x, y);

	g_pSurface->DrawPrintText(_Input, wcslen(_Input));
}
